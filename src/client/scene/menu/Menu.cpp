...
{
    int width_button = 657;
    int height_button = 284;
    float scale_button = 0.4;
    ...
    // Play button
    Vector2f pos = {static_cast<float>(window.getSize().x / 2 - ((width_button * scale_button) / 2)), 250};
    _world.create_button(
        "assets/background/bg-boutton.png", Vector4i{41, 28, width_button, height_button}, Vector2f{scale_button, scale_button}, pos, &go_to_lobby);
    _world.create_text("PLAY", "assets/font/EMINOR-BlackItalic.ttf", 40,
        Vector2f{static_cast<float>(window.getSize().x / 2 - 40), 290});

    // Option button
    pos = {static_cast<float>(window.getSize().x / 2 - ((width_button * scale_button) / 2)), 400};
    _world.create_button(
        "assets/background/bg-boutton.png", Vector4i{41, 28, width_button, height_button}, Vector2f{scale_button, scale_button}, pos, &go_to_option);
    _world.create_text("OPTION", "assets/font/EMINOR-BlackItalic.ttf", 40,
        Vector2f{static_cast<float>(window.getSize().x / 2 - 50), 440});

    // Quit button
    pos = {static_cast<float>(window.getSize().x / 2 - ((width_button * scale_button) / 2)), 550};
    _world.create_button(
        "assets/background/bg-boutton.png", Vector4i{41, 28, width_button, height_button}, Vector2f{scale_button, scale_button}, pos, &quit_callback);
