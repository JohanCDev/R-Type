...
size_t World::create_drawable_object(std::string asset_path, Vector4i rect, Vector2f scale, Vector2f pos, Vector2i speed, float refresh_time, float elapsed_time)
{
    Entity ent = this->_r.spawn_entity();

    this->_r.add_component<DrawableComponent>(ent, DrawableComponent(asset_path, rect, scale));
    this->_r.add_component<PositionComponent>(ent, PositionComponent(pos));
    this->_r.add_component<HealthComponent>(ent, (HealthComponent(1)));
